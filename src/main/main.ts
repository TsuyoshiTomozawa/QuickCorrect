...
async function createWindow(): Promise<void> {
  ...
  // ウィンドウ位置の復元
  if (
    settings.windowSettings.position.x !== -1 &&
    settings.windowSettings.position.y !== -1
  ) {
  ...
  mainWindow.on("moved", async () => {
    if (mainWindow) {
      ...
      const currentSettings =
        await settingsManager.getSetting("windowSettings");
...
async function setupIPC(): Promise<void> {
  ...
  ipcMain.handle("get-statistics", () => {
    return {
      ...
      health: correctionController
        ? Object.fromEntries(correctionController.checkProvidersHealth())
        : {},
    };
  });
  ...
  ipcMain.handle("debug-info", () => {
    ...
    return {
      workflowState:
        workflowOrchestrator?.getWorkflowState() || "not-initialized",
